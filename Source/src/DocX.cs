...
namespace DXPlus
{
    ...
    public sealed class DocX : Container, IDisposable
    {
        ...
        public void AddCoreProperty(string propertyName, string propertyValue)
        {
            ...
            {
                XNamespace propertyNamespace = corePropDoc.Root.GetNamespaceOfPrefix(propertyNamespacePrefix);
        ...
        public void AddCustomProperty(CustomProperty cp)
        {
            ...
            XElement customProperty = customPropDoc.LocalNameDescendants("property")
        ...
        {
            System.Collections.ObjectModel.ReadOnlyCollection<Paragraph> allParas = Paragraphs;
            ...
            foreach (Paragraph para in allParas)
            {
                ...
                {
                    parasInASection.Add(para);
                    Section section = new Section(Document, sectionInPara) { SectionParagraphs = parasInASection };
                    sections.Add(section);
                    parasInASection = new List<Paragraph>();
                }
            }

            Section baseSection = new Section(Document, SectPr) { SectionParagraphs = parasInASection };
        ...
        public void InsertDocument(DocX otherDoc, bool append = true)
        {
            ...
            foreach (PackagePart remote_pp in ppc)
            {
                ...
                {
                    PackagePart packagePart = ClonePackagePart(remote_pp);
                    switch (remote_pp.ContentType)
                    {
                        case "application/vnd.openxmlformats-officedocument.wordprocessingml.endnotes+xml":
            ...
            foreach (PackageRelationship hyperlink_rel in otherDoc.packagePart.GetRelationshipsByType("http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink"))
            {
                string old_rel_Id = hyperlink_rel.Id;
                string new_rel_Id = packagePart.CreateRelationship(hyperlink_rel.TargetUri, hyperlink_rel.TargetMode, hyperlink_rel.RelationshipType).Id;
                foreach (XElement hyperlink_ref in remote_mainDoc.Descendants(DocxNamespace.Main + "hyperlink"))
                {
            ...
            ////ole object links
            foreach (PackageRelationship oleObject_rel in otherDoc.packagePart.GetRelationshipsByType("http://schemas.openxmlformats.org/officeDocument/2006/relationships/oleObject"))
            {
                string old_rel_Id = oleObject_rel.Id;
                string new_rel_Id = packagePart.CreateRelationship(oleObject_rel.TargetUri, oleObject_rel.TargetMode, oleObject_rel.RelationshipType).Id;
                foreach (XElement oleObject_ref in remote_mainDoc.Descendants(XName.Get("OLEObject", "urn:schemas-microsoft-com:office:office")))
                {
            ...
            int id = 0;
            foreach (XElement local_docPr in mainDoc.Root.Descendants(DocxNamespace.WordProcessingDrawing + "docPr"))
            {
            ...
            foreach (XElement docPr in remote_body.Descendants(DocxNamespace.WordProcessingDrawing + "docPr"))
            {
        ...
        {
            TableOfContents toc = TableOfContents.CreateTableOfContents(this, title, switches, headerStyle, maxIncludeLevel, rightTabPos);
            Xml.Add(toc.Xml);
            return toc;
        }
...
.Remove();
        ...
        internal static void UpdateCustomPropertyValue(DocX document, string customPropertyName, string customPropertyValue)
        {
            ...
            string matchCustomPropertyName = customPropertyName;
                    ...
                    if (attr_value.Equals(match_value, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        while (true)
                        {
                            if (node.NodeType == XmlNodeType.Element)
                            {
                                ...
                                IEnumerable<XElement> match = ele.Descendants(DocxNamespace.Main + "t");
                                if (match.Any())
                                {
                                    if (!found)
                                    {
                                ...
                                else
                                {
                                    ...
                                    if (match.Any())
                                    {
                                        XAttribute endMatch = match.First().Attribute(DocxNamespace.Main + "fldCharType");
        ...
        internal void AddHyperlinkStyleIfNotPresent()
        {
            ...
            if (styles == null)
            {
                using StreamReader sr = new StreamReader(stylesPart.GetStream(FileMode.Open, FileAccess.Read));
        ...
        internal string GetCollectiveText(List<PackagePart> list)
        {
            ...
            foreach (PackagePart hp in list)
            {
        ...
        private static void PopulateDocument(DocX document, Package package)
        {
            ...
            foreach (PackageRelationship rel in document.packagePart.GetRelationships())
            {
        ...
        private void ClonePackageRelationship(DocX remote_document, PackagePart pp, XDocument remote_mainDoc)
        {
            ...
            foreach (PackageRelationship remote_rel in remote_document.packagePart.GetRelationships())
            {
        ...
        private void MergeFonts(DocX remote)
        {
            ...
            IEnumerable<XElement> remote_fonts = remote.fontTable.Root.Elements(DocxNamespace.Main + "font");
            IEnumerable<XElement> local_fonts = fontTable.Root.Elements(DocxNamespace.Main + "font");
        ...
        private void MergeNumbering(XDocument remote_mainDoc, DocX remote)
        {
            ...
            List<XElement> remote_abstractNums = remote.numbering.Root.Elements(DocxNamespace.Main + "abstractNum").ToList();
            int guidd = 0;
            ...
            List<XElement> remote_nums = remote.numbering.Root.Elements(DocxNamespace.Main + "num").ToList();
            int guidd2 = 0;
